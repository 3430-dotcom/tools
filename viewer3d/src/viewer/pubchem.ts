...
export async function fetchCompoundByName(name: string): Promise<CompoundLookupResult> {
  ...
  // does above.
  const info = await fetchCompoundProperties(encoded).catch(
    (): CompoundInfo => ({ molecularFormula: null, molecularWeight: null, iupacName: null }),
  )

  return { text: sdfToLegacyPDB(text), info }
}

async function fetchCompoundProperties(encodedName: string): Promise<CompoundInfo> {
  ...
  if (!res.ok) return { molecularFormula: null, molecularWeight: null, iupacName: null }
